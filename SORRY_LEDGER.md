...
> **Rule 4**: Each entry must include: theorem name, reason, planned closing SPEC/Task.

## Blocking-Path Theorems (ZERO sorry tolerance)

The following 18 theorems are on the blocking path. `sorry` is forbidden:
...
<!-- Add entries below as they are registered.
Format:
...
*No entries yet. This section will be populated as non-blocking sorrys are registered during implementation.*
